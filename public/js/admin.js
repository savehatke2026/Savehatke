...
// SELL WHITELIST
// ════════════════════════════════════════════════════════════════════════
// Emails allowed to see and use the coupon selling form. Loaded/saved with
// the same textarea-per-line pattern as the maintenance whitelist.

/**
 * Load the sell whitelist into the textarea (one email per line).
 * Called when the Settings section is opened.
 */
async function loadSellWhitelist() {
  const textarea = document.getElementById('sellWhitelist');
  const countEl = document.getElementById('sellWhitelistCount');
  if (!textarea) return;

  try {
    const data = await api('/admin/sell/whitelist', { useAdmin: true });
    const emails = Array.isArray(data && data.emails) ? data.emails : [];
    textarea.value = emails.join('\n');
    if (countEl) {
      countEl.textContent = emails.length
        ? `${emails.length} whitelisted seller(s)`
        : 'No whitelisted sellers yet';
    }
  } catch (err) {
    console.warn('Failed to load sell whitelist:', err.message);
    if (countEl) countEl.textContent = 'Could not load the whitelist.';
  }
}

/**
 * Save the textarea contents as the full replacement sell whitelist.
 * Empty / whitespace-only lines are dropped; validation is server-side.
 */
async function saveSellWhitelist() {
  const textarea = document.getElementById('sellWhitelist');
  const countEl = document.getElementById('sellWhitelistCount');
  const spinner = document.getElementById('sellWhitelistSpinner');
  if (!textarea) return;
  ...
  try {
    const data = await api('/admin/sell/whitelist', {
      method: 'PUT',
      useAdmin: true,
      body: { emails },
    });
    ...
    const saved = Array.isArray(data && data.emails) ? data.emails : [];
    textarea.value = saved.join('\n');
    if (countEl) countEl.textContent = `${saved.length} whitelisted seller(s)`;

    if (typeof showToast === 'function') {
      showToast(data.message || `Whitelist saved — ${saved.length} seller(s).`, 'success');
    }
  } catch (err) {
    if (typeof showToast === 'function') {
      showToast(err.message || 'Failed to save the whitelist.', 'error');
