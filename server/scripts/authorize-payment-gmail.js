// ============================================
// SaveHatke — one-time Payment Mailbox (Gmail) authorization
// ============================================
...
// Usage:
//   cd server
//   node scripts/authorize-payment-gmail.js
//
// Requires in .env: PAYMENT_GMAIL_CLIENT_ID / PAYMENT_GMAIL_CLIENT_SECRET
// (or the shared GMAIL_CLIENT_ID / GMAIL_CLIENT_SECRET, or
...
function fail(msg) {
  console.error('\n[authorize-payment-gmail] ' + msg + '\n');
  process.exit(1);
}

if (!CLIENT_ID || !CLIENT_SECRET) {
  fail(
    'PAYMENT_GMAIL_CLIENT_ID / PAYMENT_GMAIL_CLIENT_SECRET (or GMAIL_* / GOOGLE_* fallbacks) missing from .env'
  );
}
...
function waitForCallback() {
  ...
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      ...
      res.end(
        code
          ? '<h2>SaveHatke payment mailbox authorized</h2><p>You can close this tab and return to the terminal.</p>'
...
(async () => {
  console.log('\n=== SaveHatke Payment Mailbox authorization (no database) ===');
  console.log('Redirect URI  :', REDIRECT_URI);
  console.log('Scopes        :', SCOPES.join(' '));
  console.log('Payment inbox :', EXPECTED || '(PAYMENT_MAILBOX_EMAIL not set — any Gmail account will be accepted)');
  ...
  console.log('2. Open this URL and sign in with the PAYMENT mailbox account');
  console.log('   (the inbox that RECEIVES the UPI / bank credit emails):\n');
  console.log(authUrl + '\n');
  ...
  if (EXPECTED && address && address !== EXPECTED) {
    console.warn(
      `\n⚠️  WARNING: you authorized ${address} but PAYMENT_MAILBOX_EMAIL is ${EXPECTED}.` +
        '\n   The verifier would then read the wrong inbox. Re-run and pick the payment account.'
    );
  }

  console.log('\nAdd this line to your .env (project root) and to Vercel →');
  console.log('Project → Settings → Environment Variables:\n');
  console.log('PAYMENT_GMAIL_REFRESH_TOKEN=' + tokens.refresh_token);
  if (!EXPECTED && address) {
    console.log('PAYMENT_MAILBOX_EMAIL=' + address);
  }
  console.log('\nThen restart / redeploy the server. The payment verifier will read');
  console.log('this dedicated inbox on every boot — no database, no re-consent.\n');
})();
